...
const ESTIMATED_FEE = 1000;

send = async (amount, memo, hash, user, subtractFee = false) => {
  let payreq = bolt11.decode(hash);
  let routingInfo = payreq.tags.find(t => t.tagName === "routing_info");
  if (
    routingInfo &&
    routingInfo.data.find(d => d.fee_proportional_millionths > 1000)
  )
  ...
  if(subtractFee) {
    ...
    if(amount < 0)
      throw new Error('Subtracted fee is greater than amount to send')
  }
  ...
  await db.transaction(async transaction => {
    ...
    if (config.lna.clightning) {
      ...
      if (!m) throw new Error("lightning payment failed");
