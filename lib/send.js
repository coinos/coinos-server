...
export default async (amount, memo, hash, user) => {
  ...
  await db.transaction(async transaction => {
    ...
    let m;
    if (config.lna.lnd) {
      m = await payViaPaymentRequest({
        lnd,
        request: hash,
        max_fee: Math.min(10000, account.balance)
      });

      if (!m) throw new Error(`lightning payment failed ${amount} ${hash}`);
      if (m.payment_error) throw new Error(m.payment_error);
      ({ fee, secret, tokens: total } = m);
      preimage = secret.toString("hex");
    } else {
      let { msatoshi, msatoshi_sent, payment_preimage } = await ln.pay(hash, tokens ? `${tokens}sats` : undefined);
      preimage = payment_preimage;
      fee = parseInt((msatoshi_sent - msatoshi) / 1000);
      total = parseInt(msatoshi / 1000);
    }
