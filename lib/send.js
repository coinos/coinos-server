...
import { computeConversionFee, conversionFeeReceiver } from '../routes/lightning/conversionFee.js';
...
export const send = async (amount, memo, hash, user) => {
  ...
  await db.transaction(async transaction => {
    l.info("starting lightning send transaction");
    let account;
    ...
    if (!account || account.balance < amount + conversionFee - conversionFeeDeduction) {
      l.warn("user attempting to send more than they have", user.id, amount, conversionFee);
  ...
  await db.transaction(async transaction => {
    l.info("starting lightning fee transaction");
