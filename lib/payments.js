...
export let credit = async (hash, amount, memo, ref, type = types.internal) => {
  ...
  if (p.confirmed) autowithdraw(user);

  return p;
};

export let autowithdraw = async (user) => {
  try {
    let { username, autowithdraw, id, destination, threshold, reserve } = user;
    if (autowithdraw) {
      let to = destination.trim();
      let balance = await g(`balance:${id}`);
      let amount = balance - reserve;
      console.log("AMT", amount, balance, reserve);
      if (balance > threshold) await pay({ amount, to, user });
    }
  } catch (e) {
    warn(username, "autowithdraw failed", e);
  }
};
...
let pay = async ({ amount, to, user }) => {
  ...
  if (lnurl) {
    ...
    ({ pr } = await got(`${callback}?amount=${amount}`).json());
  ...
  if (pr) await sendLightning({ user, pr, amount });
  else await sendOnchain({ amount, address: to, user, subtract: true });
};
