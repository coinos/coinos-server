...
export let credit = async (hash, amount, memo, ref, type = types.internal) => {
  ...
  if (user.hasprinter) {
    let fiat = parseFloat(((amount * p.rate) / 100000000).toFixed(2));
    let words = dollarsToWords(fiat);

    let client = mqtt.connect("mqtt://ln.coinos.io", config.mqtt);
    client.subscribe(username);

    if (fiat === 0.02) {
      words = "there is no second best";
      client.publish(username, "easter");
    } 

    if (fiat === 0.21) {
      words = "tick tock, next block";
      client.publish(username, "easter");
    } 

    if (fiat === 0.69) {
      words = "have fun staying poor";
      client.publish(username, "easter");
    } 
    ...
    let raw = `/app/data/uploads/${p.id}-raw.mp3`;
    let out = `/app/data/uploads/${p.id}.mp3`;

    let connected;
    let mimic = spawn("/mimic", [
      "-t",
      words,
      "-voice",
      "/cmu_us_axb.flitevox",
