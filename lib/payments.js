...
export let credit = async (hash, amount, memo, ref, type = types.internal) => {
  ...
  if (user.hasprinter) {
    ...
    const mimic = spawn("/mimic", ["-t", words, "-voice", "/cmu_us_axb.flitevox", "-o", raw]);
    let connected;
    let f = fs.createWriteStream(out);
    ...
    mimic.stdout.on("end", async () => {
      ...
      let sentChunks = {}; // Dictionary to store chunks and their respective sequence numbers.

      function retransmit(chunkNumber) {
        if(sentChunks[chunkNumber]){
          console.log("retransmitting", chunkNumber)
          client.publish(username, chunkNumber.toString());
          client.publish(username, sentChunks[chunkNumber]);
          setTimeout(() => {
            if (sentChunks[chunkNumber]) { // Retransmit if not acknowledged
              retransmit(chunkNumber);
            }
          }, 5000); // Check for retransmission every 5 seconds
        }
      }

      client.on('message', (topic, message) => {
        if (message.length < 20) console.log(message, message.toString());
        if (topic === username && message.toString().startsWith('ack:')) {
          let seqNumber = parseInt(message.split(':')[1]);
          console.log("ACK", seqNumber);
          delete sentChunks[seqNumber]; // Remove acknowledged chunk from dictionary
          if (!Object.keys(sentChunks).length) client.publish(username, "end");
        }
      });
      ...
      ffmpeg(raw)
          ...
          .on('data', (chunk) => {
            let seq = n;
            client.publish(username, n.toString());
            sentChunks[n] = chunk;
            client.publish(username, chunk);
            f.write(chunk);
            setTimeout(() => {
              if (sentChunks[seq]) { // Retransmit if not acknowledged
                retransmit(seq);
              }
            }, 5000);
            n++;
    ...
    client.on('connect', function () {
      client.publish(username, `${p.amount}:${p.tip}:${p.rate}:${p.created}:${p.id}`);
