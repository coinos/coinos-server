...
import { balance, melt, mint } from "$lib/ecash";

let { URL } = process.env;

setTimeout(
  async () => console.log("bob", await balance(await getUser("bob"))),
  1000,
);
...
export let debit = async ({
  ...
  await melt(user, amount + fee + tip + ourfee);
...
export let credit = async (hash, amount, memo, ref, type = types.internal) => {
  ...
  if (!memo) ({ memo } = inv);
  if (memo && memo.length > 200) fail("memo too long");
