...
app.register(proxy, {
  upstream: config.nostr,
  prefix: "/nostr",
  rewritePrefix: "",
  disableRequestLogging: true
});

export const pool = RelayPool(config.relays);
...
let timeouts = {};
export let q = async (sub, query, timeout = 20000) =>
  new Promise(async (r, j) => {
    ...
    let check = s => {
      ...
      if (done[s].length === pool.relays.length || elapsed > 200) {
        r();
