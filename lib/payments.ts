...
export const credit = async ({
  ...
  // each runs the credit below — the /replay double-credit race. SET NX is
  // atomic, so only the first caller claims the settlement and credits; the rest
  // bail here, before tbCredit. `ref` is the lightning preimage, unique per
...
const finalize = async (r, p) => {
  ...
  // Robust against zero-amount invoices (decoded amount_msat is undefined, fall
  // back to invoice_amount_msat then r.amount_msat) and against CLN occasionally
  // returning unexpected response shapes — if we can't compute a finite integer
  // fee, keep p.fee at the originally-reserved maxfee so the subsequent refund
  // delta is 0 instead of NaN (which would crash the tbRefund balance write and
  // leave finalize in a partial state).
  const { amount_msat, invoice_amount_msat } = await ln.decode(p.hash);
  const invMsat = Number(amount_msat ?? invoice_amount_msat ?? r.amount_msat);
  ...
  if (!Number.isFinite(computedFee)) {
    warn("finalize: non-finite fee compute, keeping reserved maxfee", p.id,
         "sent_msat=", r.amount_sent_msat, "inv_msat=", amount_msat ?? invoice_amount_msat ?? r.amount_msat);
  }
