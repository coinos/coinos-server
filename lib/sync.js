...
const sync = async () => {
  ...
  try {
    ...
    let x = invoices.find(i => i.request.includes('lnbc10100u1p333l3lpp5znwsrpr9g'));
    let cancelled = invoices.filter(i => i.is_canceled);
    ...
    for (let i = 0; i < cancelled.length; i++) {
      ...
      if (settled.includes(c.secret)) {
        await db.transaction(async transaction => {
          ...
          console.log("destroying", p.id);
          await p.account.decrement({ balance: p.amount }, { transaction });
