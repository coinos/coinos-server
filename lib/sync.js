...
import { adminAuth } from "$lib/passport";
...
app.post("/renameImages", adminAuth, async (req, res) => {
  let d = './uploads/';
  fs.readdir(d, (err, files) => {
    files.forEach(async file => {
      try {
        let m = file.match(/(.*)-profile/);

        let username;

        if (m && m[1]) username = m[1];
        else m = file.match(/(.*)-banner/);
        if (m && m[1]) username = m[1];

        console.log(username);

        if (username) {
          let { uuid } = await db.User.findOne({ where: { username } });
          fs.rename(d + file, d + uuid + file.replace(username, ""), function(err) {
