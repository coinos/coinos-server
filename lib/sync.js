  ...
  try {
      ...
      try {
        ...
        if (invoice && invoice.user_id) {
          ...
          if (account) {
            ...
            l.info("lightning missing from account", account.id, amount);
          }
        }
      } catch (e) {
        l.error("problem syncing lightning payment", JSON.stringify(missed[i]), e.message, e.stack);
      }
    }
    ...
    for (let i = 0; i < missed.length; i++) {
      ...
      try {
        ...
        l.error("problem confirming payment", p, e.message, e.stack);
      }
    }
    ...
    transactions.map(tx => {
      if (
        ...
        !unaccounted.find(a => a.txid === tx.txid) &&
    ...
    for (let i = 0; i < invoices.length; i++) {
      ...
      l.info("unaccounted", address);

      let pending = unaccounted.filter(
      ...
      for (let i = 0; i < pending.length; i++) {
        ...
        try {
          await db.transaction(async transaction => {
            l.info("found pending transaction", user.username, amount, hash);
            payments.push(hash);
          ...
          l.error(
            "problem updating pending transaction",
            hash,
            e.message,
            e.stack
          );
