...
module.exports = async ({ address, asset, amount, feeRate, user }) => {
  ...
  if (!feeInput) throw new Error("Couldn't find fee input");
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    psbt = psbt.addInput({
      hash: txid,
      index: vout,
      nonWitnessUtxo: Buffer.from(await lq.getRawTransaction(txid), "hex"),
    });
  }

  if (asset !== btc) {
    l.info("feeInput", feeInput);
    psbt.addInput({
      hash: feeInput.txid,
      index: feeInput.vout,
      nonWitnessUtxo: Buffer.from(
        await lq.getRawTransaction(feeInput.txid),
        "hex"
      ),
    });
