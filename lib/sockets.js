      ...
      switch (type) {
        ...
        case "login":
          ...
          try {
            ...
            let { id: user_id } = jwt.decode(token);
            if (!user_id) throw new Error("Invalid JWT token");
            let user = await g(`user:${user_id}`);
            if (!user) throw new Error("User not found");
            user.haspin = !!user.pin;

            if (!store.sockets[user_id]) store.sockets[user_id] = {};
            const existing = Object.keys(store.sockets[user_id]);
            if (existing.length > 4) {
              const p = existing.find(sid => sid !== user_id);
              store.sockets[user_id][p].send(JSON.stringify({ type: "logout" }));
              setTimeout(() => {
                if (store.sockets[user_id][p])
                  store.sockets[user_id][p].close(code, "too many sockets");
              }, 5000);
            }
            store.sockets[user_id][id] = ws;
            names[id] = user_id;
