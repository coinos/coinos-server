  ...
  if (type === "payment" && data.amount > 0) {
    for (let i = subscriptions.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        s.invoice.address === data.address ||
        s.invoice.unconfidential === data.address
      ) {
  ...
  ws.on("message", async message => {
    ...
    switch (type) {
      ...
      case "login":
        ...
        if (differenceInSeconds(new Date(), subSeconds(lastSeen[token], 5)) < 2) return;
        lastSeen[token] = new Date();


        try {
          if (!token || token === "null") return ws.close();
