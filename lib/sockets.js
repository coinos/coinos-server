  ...
  ws.on("message", async message => {
    ...
    switch (type) {
      ...
      case "login":
        ...
        try {
          if (!token || token === "null" || !jwt.decode(token)) return ws.close(code, `bad token ${token}`);
          ...
          if (existing.length > 4) {
            ...
            setTimeout(() => {
              if (sockets[username][p]) sockets[username][p].close(code, "too many sockets");
            }, 5000);
          }
          ...
          if (e.message.includes("not found"))
            ws.send(JSON.stringify({ type: "logout" }));
          setTimeout(() => ws && ws.close(code, `closing due to error ${e.message}`), 1000);
        }
        break;
      case "token":
        if (!ws.user) return;
