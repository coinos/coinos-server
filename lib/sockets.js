import store from "./store";
import config from "$config";
import app from "$app";
import jwt from "jsonwebtoken";
import { v4 } from "uuid";
import WebSocket from "ws";
import { differenceInSeconds, subSeconds } from "date-fns";
import fastifyWs from "@fastify/websocket";
import whitelist from "./whitelist";
import { l, err, warn } from "$lib/logging";
import { getUser } from "$lib/utils";
import db from "$db";

const code = 1000;

const wss = new WebSocket.Server({ clientTracking: false, noServer: true });

const pick = (O, ...K) => K.reduce((o, k) => ((o[k] = O[k]), o), {});
const all = {};
const subscriptions = [];
const lastSeen = {};

export const emit = (username, type, data) => {
  if (data.username !== undefined) data = pick(data, ...whitelist);

  if (type === "payment" && data.amount > 0) {
    for (let i = subscriptions.length - 1; i >= 0; i--) {
      let s = subscriptions[i];

      if (s.invoice.id === data.invoice_id) {
        s.ws.send(JSON.stringify({ type, data }));
      }
    }
  }

  if (!store.sockets[username]) return;
  for (let id in store.sockets[username]) {
    const ws = store.sockets[username][id];
    ws.send(JSON.stringify({ type, data }));
  }
};

export const broadcast = (type, data) => {
  const sent = [];
  for (const username in store.sockets) {
    for (const id in store.sockets[username]) {
      const ws = store.sockets[username][id];
      ws.send(JSON.stringify({ type, data }));
      sent.push(ws.id);
    }
  }

  Object.values(all).map(ws => {
    if (!sent.includes(ws.id)) ws.send(JSON.stringify({ type, data }));
  });
};

function subtractSeconds(numOfSeconds, date = new Date()) {
  date.setSeconds(date.getSeconds() - numOfSeconds);

  return date;
}

app.register(fastifyWs);
app.register(async function(app) {
  app.get("/ws", { websocket: true }, ({ socket: ws }, req) => {
    const id = v4();
    ws.id = id;
    ws.beats = 0;
    ws.send(JSON.stringify({ type: "connected", data: id }));
    all[id] = ws;

    ws.on("message", async message => {
      let type, data;
      try {
        ({ type, data } = JSON.parse(message));
      } catch (e) {
        err("coudn't parse socket message");
      }

      switch (type) {
        case "heartbeat":
          ws.beats = 0;
          ws.send(JSON.stringify({ type: "id", data: ws.id }));
          break;

        case "login":
          let user;
          let token = data;
          if (lastSeen[token] > subtractSeconds(5)) return;
          lastSeen[token] = new Date();

          try {
            if (!token || token === "null" || !jwt.decode(token))
              return ws.close(code, `bad token ${token}`);
            let { username, uuid } = jwt.decode(token);
            if (uuid) user = await getUser(uuid);
            username = user.username;
            if (!(username || uuid)) throw new Error("Invalid JWT token");

            if (!store.sockets[username]) store.sockets[username] = {};
            const existing = Object.keys(store.sockets[username]);
            if (existing.length > 4) {
              const p = existing.find(sid => sid !== id);
              store.sockets[username][p].send(
                JSON.stringify({ type: "logout" })
              );
              setTimeout(() => {
                if (store.sockets[username][p])
                  store.sockets[username][p].close(code, "too many sockets");
              }, 5000);
            }
            store.sockets[username][id] = ws;
            names[id] = username;

            if (!user) user = await getUser(username);

            let { haspin } = user;

            if (user.locked) throw new Error("Account is locked");
            let payments = await user.getPayments({
              where: {
                account_id: user.account_id
              },
              order: [["id", "DESC"]],
              limit: 12,
              include: [
                {
                  model: db.Account,
                  as: "account"
                },
                {
                  model: db.Payment,
                  as: "fee_payment"
                }
              ]
            });

            let accounts = await user.getAccounts();

            let keys = await user.getKeys();
            user = user.get({ plain: true });
            user.accounts = accounts;
            user.keys = keys;
            user.payments = payments;
            user.haspin = haspin;

            ws.user = user;
            ws.send(
              JSON.stringify({ type: "login", data: pick(user, ...whitelist) })
            );
            ws.send(
              JSON.stringify({ type: "version", data: config.clientVersion })
            );
          } catch (e) {
            err("caught socket error", e.message);
            if (e.message.includes("locked")) {
              ws.send(JSON.stringify({ type: "locked" }));
              ws.send(JSON.stringify({ type: "logout" }));
            }
            if (e.message.includes("not found"))
              ws.send(JSON.stringify({ type: "logout" }));
            setTimeout(
              () => ws && ws.close(code, `closing due to error ${e.message}`),
              1000
            );
          }
          break;
        case "token":
          if (!ws.user) return;
          ws.token = data;
          break;
        case "lnurl":
          store.sessions[data] = ws;
          break;
        case "updateMemo":
          const payment = await db.Payment.findOne({
            where: {
              user_id: ws.user.id,
              id: data.id
            },
            include: {
              model: db.Account,
              as: "account"
            }
          });
          if (payment) {
            payment.memo = data.memo;
            await payment.save();
            emit(ws.user.username, "payment", payment);
          }
          break;
        case "subscribe":
          subscriptions.push({ invoice: data, ws });
          break;
        default:
          warn("received socket message of unknown type", type, data);
      }
    });

    ws.on("close", function(code, reason) {
      // console.log("closing socket", reason && reason.toString(), id);
      try {
        const username = names[id];
        if (store.sockets[username] && store.sockets[username][id]) {
          delete store.sockets[username][id];
        }

        if (all[id]) delete all[id];
      } catch (e) {
        err("problem closing socket", e.message);
      }
    });
  });
});

const names = {};

setInterval(() => {
  for (const username in store.sockets) {
    for (const id in store.sockets[username]) {
      let ws = store.sockets[username][id];
      ws.beats++;
      if (ws.beats > 10) ws.close(code, `${username} ${id} lost heartbeat`);
    }
  }
}, 2000);
