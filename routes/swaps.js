...
const { Swap, WatchOnlyWallet, fetchUtxos, networks } = require("tdex-sdk");
    ...
    try {
      ...
      */
      ...
      const woWallet = new WatchOnlyWallet({
        address: (await lq.getAddressInfo(await lq.getNewAddress("", "bech32")))
          .unconfidential,
        network
      });

      let utxos = (
        await lq.listUnspent(0, 9999999, [], false, { asset: a1 })
      ).map(({ txid, vout, amount, asset }) => ({
        txid,
        vout,
      ...
      let psbtBase64 = woWallet.updateTx(
        emptyPsbt,
        utxos,
        Math.round(v1 * SATS),
        Math.round(v2 * SATS),
      ...
      let { Psbt } = liquid;
      let psbtObj = Psbt.fromBase64(psbtBase64);
      console.log(psbtBase64);
      console.log("data", psbtObj.data);
      psbtObj.data.outputs.map(o => {
      ...
      const text = swap.request({
        psbtBase64,
        amountToBeSent: Math.round(v1 * SATS),
        amountToReceive: Math.round(v2 * SATS),
        assetToBeSent: a1,
        assetToReceive: a2
      });

      let filename = swapsdir + "swap_request.bin";
      ...
      let swapRequest,
        serializedSwapRequest,
        walletInstance,
        swapAccept,
        swapAcceptFile;

      serializedSwapRequest = await fs.readFileSync(filename);
      ...
      console.log(
        `SwapRequest message: ${JSON.stringify(swapRequest, undefined, 2)}`
      );

      const toWallet = new WatchOnlyWallet({
        address: (await lq.getAddressInfo(await lq.getNewAddress("", "bech32")))
          .unconfidential,
        network
      });

      utxos = (await lq.listUnspent(0, 9999999, [], false, { asset: a2 })).map(
        ({ txid, vout, amount, asset }) => ({
          txid,
          vout,
          value: Math.round(amount * SATS),
          asset
      ...
      psbtObj = Psbt.fromBase64(swapRequest.transaction);
      console.log("data", psbtObj.data);
      psbtObj.data.outputs.map(o => {
        console.log("output", o);
      });
      ...
      let { psbt } = await lq.walletSignPsbt(psbtObj.toBase64());
      let tx = await lq.finalizePsbt(psbt);
      console.log(tx);
      await lq.sendRawTransaction(tx.hex);

      // toWallet.sign(psbtBase64);
