...
router.post("/grant", auth, async (req, res) => {
  ...
  var ref = await knex.table("referrals").insert({
...
router.get("/checkTokens/:sponsor_id", auth, async (req, res) => {
  ...
  if (status && status !== "all")
    tokens = tokens.where("status", "like", status);
 ...
 */
router.get("/verify/:user_id/:token", auth, async (req, res) => {
...
router.post("/joinQueue", async (req, res) => {
  ...
  await knex.table("waiting_list").insert({
    email: email,
    phone: phone,
    user_id: user_id
  });
  ...
  res.send({
    success: true,
    message: "Added " + email + " to waiting list " + phone
  });
});

router.get("/joinQueue", async (req, res) => {
 ...
 */
router.get("/isReferred/:user_id", async (req, res) => {
